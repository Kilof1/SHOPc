...
const Currency = () => {
    ...
    const [currency, setCurrency] = React.useState<currencyTypes>(availableCurrencies[0]);
    ...
    <>
        <span className='currency' onClick={showMenu}>
            <img className='currency__image' src={currency.img} alt="Niestety"/> <span className='currency__name'>{currency.cur}</span>
        </span>

        {
            menu 
                ? (
                    ...
                    <div key={c.cur} className="menu__option" onClick={()=> changeCurrency(c.cur, c.img)}>
